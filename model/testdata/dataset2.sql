...
CREATE DATABASE `myForum` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `myForum`;
...
CREATE TABLE `opinions` (
  ...
  `opinionstate_id` int(11) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_opinions_topics1_idx` (`topic_id`),
  KEY `fk_opinions_users1_idx` (`user_id`),
  KEY `fk_opinions_opinionstates1_idx` (`opinionstate_id`),
  CONSTRAINT `fk_opinions_opinionstates1` FOREIGN KEY (`opinionstate_id`) REFERENCES `opinionstates` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
INSERT INTO `opinions` VALUES (1,'C\'est important !',4,4,1),(2,'Ed est évidemment supérieur à tout les autres éditeurs',5,5,2),(3,'Enfin un document sur la GP qui peut se lire en moins d\'une heure !!!!',4,1,3),(4,'C\'est indispensable pour bien gérer ses projets',4,7,4),(5,'Pouvez vous être plus précis dans votre topic? A quel système de base de données souhaitez-vous vous connecter ?\nMysql ? MariaDB ? PostgreSQL ? MangoDB ?\n\n<a href=\"https://www.arangodb.com\"> ArangoDB ?</a>',1,5,5),(6,'mmm ce n\'est pas ouf d\'indenter son code, ça prendre de la place sur le disque ! ',3,4,6),(7,'C\'est pour faciliter la lisibilité!',3,7,5),(8,'Rien à voir ici ... <script>alert(\"mmmhhh\")</script>',3,5,4),(9,'Il faut faire un max de projet personnel et découvrir les différentes technologies qui sont sur la place du marché.',9,3,3),(10,'Il faut travailler dur!',9,7,2),(11,'Windows + R -> cmd -> ping 127.0.0.1',11,7,1),(12,'Je pense que JQuery a été quasi vital un temps, mais que les évolutions de JS l\'ont maintenant (2020) rendu caduc',10,1,2),(13,'il y a toujours une place ici (voir référence) pour ceux qui utilise du jQuerry en 2020',10,14,3),(14,'Il faut capturer le handshake. Et si besoin deauth les clients',11,4,4),(15,'Il faut brancher la prise d\'alimentation!',16,7,5),(16,'Personnelement j\'aime bien et je me suis déjà créé des habitudes.',10,9,6),(17,'Il faut utiliser la commande : aircrack-ng -w password.lst -b 00:14:6C:7E:40:80 psk*.cap',14,4,5),(18,'Je pense qu\'il faudrait que tu contacte un technicien ',16,14,4),(19,'C\'est pas bien compliquer il te suffit de metttre en place un vpn ou d\'utiliser un vpn gratuit comme protonVPN cela marche surper bien dans les écoles notamment ou en entreprise',15,7,3),(20,'Je ne sais pas si on peut mais je t\'invite à regarder cette vidéo',17,3,2),(21,'Tout dépend de la nature de la panne. Si c\'est hardware, pas le choix il faut réparer/remplacer. Mais si c\'est logiciel (OS), c\'est l\'occasion de questionner le choix de son OS et de se renseigner sur des alternative peut-être plus fiables',16,1,1),(22,'Je pense que sass est utile est inutile à la fois car la plupart du temps css peut faire pareil',18,6,2),(23,'J\'espère que les modérateurs de ce site supprimeront ce sujet qui est à l\'évidence une provocation gratuite',6,1,3),(24,'Je dirais même: \"Est-ce que CSS est vraiment utile?\".',18,2,4),(25,'Atom est un bon éditeur de texte, que j\'utilise et qui ne contient pas un surplus d\'information dès le démarage. On peut peu à peu rajouter des packages afin de répondre à nos besoins, ainsi que modifer ceux déjà présent.',5,9,5),(26,'Alors bien évidamment que oui, cela te permet de crée de multiple élément avec des boucles sans devoir tous écrire à la main en plus de permettre l\'inclusion de plusieurs fichier dans un seul et même fichier ce qui permet une certaine modularité',18,3,6),(27,'Je pense qu\'il faut plus avoir un jeu facile d\'utilisation que beau graphiquement.',7,6,5),(28,'VisualStudio est à mon opinion le meilleur éditeur de texte. Je l\'utilise pour tout, même pour faire ma liste de courses ! Le fait qu\'il prenne 3 minutes pour se lancer n\'est juste qu\'un tout petit inconvenient.',5,2,4),(29,'A mes yeux, de nos jours, les graphismes ansi que le gameplay sont les deux choses les plus importantes pour créer un jeu vidéo',7,15,3),(30,'J\'adore Scrum !!',4,13,2);
...
--
-- Table structure for table `opinionstates`
--

DROP TABLE IF EXISTS `opinionstates`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `opinionstates` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
--
-- Dumping data for table `opinionstates`
--

LOCK TABLES `opinionstates` WRITE;
/*!40000 ALTER TABLE `opinionstates` DISABLE KEYS */;
INSERT INTO `opinionstates` VALUES (3,'En Modification'),(2,'En revue'),(1,'Nouveau'),(6,'Obsolet'),(5,'Publié'),(4,'Rejeté');
/*!40000 ALTER TABLE `opinionstates` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `opinionstatetransitions`
--

DROP TABLE IF EXISTS `opinionstatetransitions`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `opinionstatetransitions` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `from_id` int(11) NOT NULL,
  `to_id` int(11) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `Uniquetrans` (`from_id`,`to_id`),
  KEY `fk_opinionstatetransitions_opinionstates1_idx` (`from_id`),
  KEY `fk_opinionstatetransitions_opinionstates2_idx` (`to_id`),
  CONSTRAINT `fk_opinionstatetransitions_opinionstates1` FOREIGN KEY (`from_id`) REFERENCES `opinionstates` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_opinionstatetransitions_opinionstates2` FOREIGN KEY (`to_id`) REFERENCES `opinionstates` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `opinionstatetransitions`
--

LOCK TABLES `opinionstatetransitions` WRITE;
/*!40000 ALTER TABLE `opinionstatetransitions` DISABLE KEYS */;
INSERT INTO `opinionstatetransitions` VALUES (1,1,2),(2,2,3),(4,2,4),(5,2,5),(3,3,2),(7,3,6),(6,5,6);
/*!40000 ALTER TABLE `opinionstatetransitions` ENABLE KEYS */;
UNLOCK TABLES;
...
INSERT INTO `references` VALUES (1,'Scrum guide','https://www.scrum.org/resources/scrum-guide'),(2,'Juice it or lose them','https://www.youtube.com/watch?v=Fy0aCDmgnxg'),(3,'Documentation php','https://www.php.net/'),(4,'Livre de développement','https://www.humblebundle.com/books/learn-to-code-the-fun-way-no-starch-press-books?hmb_source=humble_home&hmb_medium=product_tile&hmb_campaign=mosaic_section_2_layout_index_2_layout_type_twos_tile_index_1_c_codingbookshelfnostarchpress_bookbundle'),(5,'Hak5','https://www.youtube.com/user/Hak5Darren'),(6,'Talk sur les index SQL','https://youtu.be/HubezKbFL7E'),(7,'Trash racoon','https://mdc.mo.gov/sites/default/files/Raccoon%20garbage_08RGB.jpg'),(8,'Article','https://www.ma-no.org/en/programming/javascript/is-jquery-going-to-die-in-2019'),(9,'ProtonVPN','https://protonvpn.com/fr/'),(10,'TempleOS linus tech tip','https://www.youtube.com/watch?v=LtlyeDAJR7A');
...
-- Dump completed on 2020-11-08 14:14:24
